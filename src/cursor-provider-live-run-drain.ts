...
/**
 * The only turns that reach the provider while a run is parked on a pi bridge tool call
 * are started from inside that pi tool, so adopting the run would wait for progress
 * that only this caller can produce.
 */
